...
export function buildCampaignHtml({ intro, events = [], locale = 'pl' }) {
  const t = getDict(locale);
  const rows = events.map(e => {
    const price = e.priceFrom ? ` &middot; ${t.ticket.from} ${esc(e.priceFrom)} ${t.ticket.currency}` : '';
    const href = e.ticketUrl || `${SITE}/wydarzenie/${e.slug}`;
    const cta = e.ticketUrl ? t.mail.buy : t.mail.details;
    return `
      <tr><td style="padding:20px 0;border-top:1px solid #2C2618;">
        <div style="font:11px/1 'Courier New',monospace;color:#C09C18;letter-spacing:1.5px;text-transform:uppercase;">${esc(formatDate(e.date, locale))}</div>
  ...
  return `<!doctype html><html lang="${locale}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><meta name="color-scheme" content="dark"></head>
  <body style="margin:0;padding:0;background:#0C0A08;-webkit-text-size-adjust:100%;">
    <span style="display:none;max-height:0;overflow:hidden;opacity:0;">${esc(t.mail.preheader)}</span>
      ...
      <tr><td align="center">
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="width:600px;max-width:600px;background:#0C0A08;border:1px solid #2C2618;border-radius:8px;overflow:hidden;">
          ...
          <tr><td style="padding:22px 28px;border-top:1px solid #2C2618;font:11px/1.7 'Courier New',monospace;color:#6E6858;">
            <strong style="color:#9A9484;">Zaklęte Rewiry</strong> &middot; ${t.mail.footerLine}<br>
            ${esc(t.mail.unsubscribe)}
          </td></tr>
        </table>
      </td></tr>
    </table>
 ...
 */
export async function sendCampaign({ subject, html, locale = 'pl' }) {
  const m = getDict(locale).panel.msg;
  const key = process.env.MAILERLITE_API_KEY;
  const groupId = process.env.MAILERLITE_GROUP_ID;
  ...
  try {
    ...
    if (!createRes.ok) {
      return { ok: false, status: 502,
        ...
        message: m.createFailed };
    }

    const id = created?.data?.id;
    if (!id) return { ok: false, status: 502, reason: 'no campaign id', message: m.noId };
    ...
    if (!sendRes.ok) {
      ...
      return { ok: false, status: 502, reason: `MailerLite schedule ${sendRes.status}: ${detail.slice(0, 160)}`,
        message: m.scheduleFailed };
    }

    return { ok: true, message: m.sent };
  } catch (err) {
